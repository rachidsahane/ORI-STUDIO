...
#
# Ticket: ORI-T-0014.  Spec: spec/CI_CD.md section 1 gate 2,
# spec/runbooks/prove-gate.md.  Rule: AICD §14.
# Read by `fixtures/planted/gate-2/prove.sh`; exercised on every run against
# the samples in `fixtures/planted/gate-2/workflow-samples/`.
#
# THIS FILE IS A COPY OF `fixtures/planted/gate-1/workflow-facts.awk`, AND THE
# COPY IS DELIBERATE. ORI-T-0014 was asked to reuse gate 1's shape and to
...
#     every edit to it an edit to gate 1's proof, so a change made for gate 2
...
#     scope (`.github/workflows/ci.yml`, `fixtures/planted/gate-2/**`), and
