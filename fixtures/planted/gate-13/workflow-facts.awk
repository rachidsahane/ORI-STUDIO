...
#
# Ticket: ORI-T-0017.  Spec: spec/CI_CD.md section 1 item 13,
# spec/CONVENTIONS.md "Git", spec/runbooks/prove-gate.md.  Rule: AICD §14.
# Read by `fixtures/planted/gate-13/prove.sh`; exercised on every run against
# the samples in `fixtures/planted/gate-13/workflow-samples/`.
#
# THIS FILE IS A COPY OF `fixtures/planted/gate-7/workflow-facts.awk`, WHICH IS
# ITSELF A COPY OF GATE 1'S, AND EACH COPY IS DELIBERATE. The reasoning is
# gate 7's, unchanged and restated here because a reader of this directory
# should not have to go and find it:
#
#   - gate 1 is an INSTALLED gate (ops/gates/gate-1.md). A shared parser makes
#     every edit to it an edit to gate 1's proof, so a change made for gate 13
#     would require gate 1 to be re-proved, and gate 1's proof would stop being
#     reproducible from gate 1's own directory.
#   - a parser bug that made one gate's prover refuse would make all of them
#     refuse on the same commit, which turns one repairable failure into four
#     red gates and removes every second opinion.
#   - the shared location would be a new path outside this ticket's declared
#     scope, and so would every other copy.
#
# What a copy costs is that a defect found in one is not fixed in the others,
...
#
# That is also, exactly, the defect gate 13 exists to catch one layer down: a
# `Ticket:` line that is really body text reads the same to a grep and means
# nothing to the parser that consumes it. This file is the workflow half of
# the same discipline, and `git interpret-trailers` is the commit-message half.
#
...
#                               and gate 2's do not. Gate 7 and gate 13 are the
#                               two gates whose liveness depends on an input to
#                               an action rather than on a command: both read
#                               history, and the history is only on the runner
#                               when the checkout step is given
#                               `fetch-depth: 0`. For gate 13 the failure is
#                               sharper than for gate 7, because the base and
#                               head commits a pull request event names are
#                               simply not objects a shallow clone holds.
